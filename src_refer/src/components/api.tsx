...
/* --- Get Team and its Player List --- */
export const fetchTeamsAndPlayers = async () => {
  try {
    const { data, error } = await getHelper(table.teams, `
      id,
      name,
      players (
        id,
...
/* --- Get All Matches [Grouped by Match & Block] --- */
...
/* --- Get All Matches Data By Week and Block --- */
...
export const fetchPlayersByTeamId = async (
  teamId: string
) => {
  try {
    const { data, error } = await getHelper(table.players, sql_query.all)
  ...
  const addPlayers = async () => {
    ...
    try {
      if (mode === 'single') {
        ...
        const { error } = await insertHelper(table.players, [{ name: newPlayerName, team_id: selectedTeam }]);
...
export const fetchDashboardData = async () => {
  try {
    console.log('API-fetchDashboardData');
    const totalTeams = await getHelper(table.teams, 'id',  { count: 'exact', head: true });
    const totalPlayers = await getHelper(table.players, 'id', { count: 'exact', head: true });
...
// export const fetchLeagueList = async (userId: any) => {
//   try {
//     let query = queryHelper(table_leagues, query_select_all);
