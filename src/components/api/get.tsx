...
export const getTeamsAndPlayersByLeagueId = async (
  leagueId: any
) => {
  try {
...
/* --- GET ALL BLOCKS --- */
export const getAllBlocksByLeagueId = async (
  leagueId: any
) => {
  try {
    const { data, error } = await getHelper(table.blocks, sql_query.all)
...
/* --- GET All TEAMS --- */
export const getAllTeamsByLeagueId = async (
  leagueId: any
) => {
  try {
...
/* --- GET ALL PLAYERS --- */
export const getAllPlayersByLeagueId = async (
  leagueId: any
) => {
  try {
    ...
    const matches = data.map((row: any) => {
      const formatTeam = (team: any) => {
        const players = (team?.players || []).map((player: any) => {
          const score = player.weekly_scores?.find((s: any) => s.match_id === row.id) || {};
          return {
            id: player.id,
            name: player.name,
            g1: score.g1 || 0,
...
export const getAllWeeksOrByBlock = async (
  blockNumber: any,
  leagueId: any
) => {
  try {
...
export const getAllWeeklyScores = async (
  leagueId: any
) => {
  try {
