...
import {
  ...
  getAllPlayersByLeagueId,
  getAllTeamsByLeagueId,
  getAllWeeklyScores,
  getAllWeeksOrByBlock
} from '../api/get';
import {
  Card,
  CardContent,
...
export default function Statistics() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const allPlayers = await getAllPlayersByLeagueId(selectedLeague);
      setPlayers(allPlayers);

      const weekslist = await getAllWeeksOrByBlock(selectedBlock, selectedLeague);
      setWeeksAvailable(weekslist);

      const allScoresData = await getAllWeeklyScores(selectedLeague);
  ...
  const teamStats = useMemo(() => {
    ...
    // Group scores by match (block + week + match)
    const matches: Record<string, any> = {};
    allScoresData.forEach((score: any) => {
      if (selectedBlock !== "all" && score.block_id?.toString() !== selectedBlock) return;
      if (selectedWeek !== "all" && score.week_number?.toString() !== selectedWeek) return;

      const matchKey = `${score.block_id}_${score.week_number}_${score.match_id}`;
      if (!matches[matchKey]) matches[matchKey] = {};
      if (!matches[matchKey][score.team_id]) matches[matchKey][score.team_id] = 0;

      matches[matchKey][score.team_id] += score.scratch; // Sum scratch per team per match
    });

    // Calculate stats for each team per match
    Object.entries(matches).forEach(([matchKey, match]) => {
      const teamIds = Object.keys(match).map(Number);
      if (teamIds.length === 0) return;

      // Extract block/week/match info from key
      const [blockId, weekNumber, matchId] = matchKey.split("_").map(Number);

      // 🟢 CASE 1: Single team (vs BLIND)
      if (teamIds.length === 1) {
        ...
        if (!stats[teamId]) {
          stats[teamId] = {
            team_name:
              allScoresData.find((s: any) => s.team_id === teamId)?.team_name || "BLIND",
            total_scratch: 0,
            average: 0,
            games_played: 0,
            wins: 0,
        ...
        // Count players who actually played in this match
        const playersInTeam = allScoresData.filter(
          (s: any) =>
            s.team_id === teamId &&
            s.block_id === blockId &&
        ...
        // Update average
        stats[teamId].average = stats[teamId].games_played
          ? (stats[teamId].total_scratch / stats[teamId].games_played).toFixed(2)
          : "0.00";

        return;
      }

      // 🟣 CASE 2: Normal 2-team match
      const [team1Id, team2Id] = teamIds;
      const team1Score = match[team1Id];
      const team2Score = match[team2Id];

      // Initialize team stats if missing
      [team1Id, team2Id].forEach((id) => {
        if (!stats[id]) {
          stats[id] = {
            team_name:
              allScoresData.find((s: any) => s.team_id === id)?.team_name || "BLIND",
            total_scratch: 0,
            average: 0,
            games_played: 0,
            wins: 0,
      ...
      // Count how many players per team actually played this match
      const playersTeam1 = allScoresData.filter(
        (s: any) =>
          s.team_id === team1Id &&
          s.block_id === blockId &&
          s.week_number === weekNumber &&
      ...
      const playersTeam2 = allScoresData.filter(
        (s: any) =>
          s.team_id === team2Id &&
          s.block_id === blockId &&
          s.week_number === weekNumber &&
      ...
      // Update pin totals and games
      stats[team1Id].total_scratch += team1Score;
      stats[team2Id].total_scratch += team2Score;

      stats[team1Id].games_played += playersTeam1 * 3;
      stats[team2Id].games_played += playersTeam2 * 3;
      ...
      // } else if (team2Score === team1Score) {
      //   // 🟡 Draw — each team gets 0.5 win
      //   stats[team1Id].wins += 0.5;
      //   stats[team2Id].wins += 0.5;
      }
    });

    // 🧮 Finalize computed stats
    return Object.entries(stats)
      .map(([teamId, stat]: any) => ({
        team_id: Number(teamId),
        ...stat,
        average: stat.games_played
          ? (stat.total_scratch / stat.games_played).toFixed(2)
          : "0.00",
        winRate: stat.matches_played
          ? Number(((stat.wins / stat.matches_played) * 100).toFixed(2))
          : 0,
      }))
